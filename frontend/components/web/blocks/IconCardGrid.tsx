      ...
      <div className="container mx-auto px-6">
        {(eyebrow || title || subtitle) && (
          <header className="text-center mb-12">
            ...
            {subtitle && <p className="text-secondary-foreground/70 max-w-2xl mx-auto">{subtitle}</p>}
          </header>
        )}
          ...
          {items.map(({ icon: Icon, title: cardTitle, desc }) => (
            <li key={cardTitle}>
              <article className="rounded-2xl bg-secondary-foreground/10 border border-secondary-foreground/15 p-6 text-center hover:bg-secondary-foreground/15 transition-colors h-full">
                <div aria-hidden className="w-16 h-16 mx-auto mb-4 rounded-full bg-secondary-foreground/15 flex items-center justify-center">
