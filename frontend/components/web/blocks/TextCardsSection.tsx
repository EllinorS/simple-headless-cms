  ...
  return (
    <section className={`py-20 px-4 ${isSecondary ? 'bg-secondary text-secondary-foreground' : ''}`}>
      <div className="container mx-auto max-w-5xl">
        <div className={`grid md:grid-cols-2 gap-12 items-center ${cardPosition === 'left' ? 'md:grid-flow-dense' : ''}`}>
          ...
          {/* Cards grid */}
          <div className={`grid grid-cols-2 gap-4 ${cardPosition === 'left' ? 'md:col-start-1 md:row-start-1' : ''}`}>
            {cards.map(({ icon: Icon, label, sub }) => (
              ...
              >
                <CardContent className="p-5">
                  ...
                  <p className={`text-xs mt-1 ${isSecondary ? 'text-secondary-foreground/70' : 'text-muted-foreground'}`}>{sub}</p>
                </CardContent>
              </Card>
            ))}
          </div>
